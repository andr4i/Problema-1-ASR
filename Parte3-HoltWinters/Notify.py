...
def reporte():
    pdf = PDF()
    pdf.add_page()
    pdf.titles("Detección de un comportamineto anómalo: ")
    pdf.set_author('Gonzalo Andrei')
    txto = "Durante la monitorización del sistema se encontro un valor fuera de lo esperado."
    pdf.texts(txto)
    gr1 = "/home/gonzalo/PycharmProjects/Introduccion_SNMP/4-AdministraciónDeRendimiento/IMG/pred.png"
    pdf.grafica1(gr1)
    pdf.output('ResultadosHW.pdf','F')
...
def send_alert_attached(subject):
    ...
    files = ['/home/gonzalo/PycharmProjects/Introduccion_SNMP/4-AdministraciónDeRendimiento/Parte3-HoltWinters/ResultadosHW.pdf']
    for file in files:
...
def send_alert_attached(subject):
    ...
    newMessage['From'] = Sender_Email
    newMessage['To'] = Reciever_Email
    newMessage.set_content('Según la predicción algunos valores no encajan')
    files = ['/home/gonzalo/PycharmProjects/Introduccion_SNMP/4-AdministraciónDeRendimiento/Parte3-HoltWinters/ResultadosHW.pdf']
    for file in files:
        with open(file, 'rb') as f:
            file_data = f.read()
